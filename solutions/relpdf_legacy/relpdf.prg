...
//memvar aReport
static aReport  
procedure relpdf( lc_arq , ln_config )

   local nK // Linha
   local xASC
   local a_orientacao := { "P" , "L" , "P" , "L" , "P"  }
   local a_largura    := { 140 , 140 , 95 , 157 , 120 } 
   local a_tamfonte   := { 6 , 9 , 10 , 8 , 8 }
   DEFAULT ln_config to 1
   
   aReport := array( PARAMLEN )
   
   set date format "YYYY/MM/DD" // important for save/load array function!!!

   pdfOpen( lc_arq + '.pdf', 200, .t.)
//   pdfNewPage( "LETTER", "P", 6 )
//   pdfSetFont( "Courier" , 0/*tipo NORMAL,bold,etc*/ , 7 /*tamanho*/ )
   pdfNewPage( "LETTER", a_orientacao[ ln_config ], 6 )
   pdfSetFont( "Courier" , 0/*tipo NORMAL,bold,etc*/ , a_tamfonte[ ln_config ] /*tamanho*/ )

   

   DBCREATE( lc_arq  + ".lin" , {{"LINHA","C",300,0}} )
   CLOSE ALL
   USE ( lc_arq + ".lin"  )
   APPEND FROM ( lc_arq + ".txt" ) SDF
   GO TOP
   nK := 0
   DO WHILE !EOF()
      ++nK
      //? linha //tava saindo no cgi
      lc_linha := RetiraASCString( LINHA )
      //lc_linha := LINHA
      //FOR xASC := 0 TO 31
      ...
      //lc_linha := STRTRAN( LINHA , CHR(15) , "" )
      //lc_linha := STRTRAN( lc_linha , CHR(10) , "" )
      //lc_linha := STRTRAN( lc_linha , CHR(18) , "" )
      //lc_linha := STRTRAN( lc_linha , CHR(K_CTRL_M) , "" )
      //? lc_linha
      //pdfRJust( SUBSTR( lc_linha , 1, 137 ) , nK , 200 /*aReport[ REPORTWIDTH ]*/ , "R" )
      pdfRJust( SUBSTR( lc_linha  , 1, a_largura[ ln_config ] ) , nK , 200 /*aReport[ REPORTWIDTH ]*/ , "R" )     
      IF AT( "" , LINHA  ) <> 0
           pdfNewPage( "LETTER",  a_orientacao[ ln_config ] , 6 )
//         pdfNewPage( "LETTER", "P", 6 )
         nK := 1
   ...
   pdfClose()
   set date format "DD/MM/YYYY" // Restaurando o formato de data para o brasileiro
